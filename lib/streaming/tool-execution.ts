...
export async function executeToolCall(
  ...
  if (!coreMessages || coreMessages.length === 0) {
    console.error('Tool execution called with empty messages')
    return { toolCallDataAnnotation: null, toolCallMessages: [] }
  }

  console.log(`Tool execution called with ${coreMessages.length} messages`)
  ...
  const toolCallMessages: CoreMessage[] = [
    ...
    {
      role: 'user',
      content: 'Based on the above search results, please provide a comprehensive answer to the original question.'
    }
  ]

  console.log(`Tool execution: Returning ${toolCallMessages.length} messages`)
