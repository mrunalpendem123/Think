  ...
  return createDataStreamResponse({
    execute: async (dataStream: DataStreamWriter) => {
      ...
      try {
        ...
        console.log(`Manual stream: Converted ${coreMessages.length} messages`)
        
        const truncatedMessages = truncateMessages(
          coreMessages,
          getMaxAllowedTokens(model)
        )
        console.log(`Manual stream: Truncated to ${truncatedMessages.length} messages`)
        ...
        console.log(`Manual stream: Tool call returned ${toolCallMessages.length} additional messages`)

        const finalMessages = [...truncatedMessages, ...toolCallMessages]
        console.log(`Manual stream: Final message count: ${finalMessages.length}`)
        
        if (finalMessages.length === 0) {
          throw new Error('No messages to send to the model. Original messages might be empty.')
        }
