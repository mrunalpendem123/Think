...
const Page = () => {
  ...
  const fetchArticles = React.useCallback(async (topic: string) => {
    setLoading(true);
    try {
      ...
      // Handle empty results (rate limiting)
      if (data.blogs && data.blogs.length > 0) {
        const filtered = data.blogs.filter((blog: Discover) => blog.thumbnail);
        setDiscover(filtered);
      } else {
        setDiscover([]);
        if (data.message) {
          toast.warning(data.message || 'No articles found.');
        }
      }
    } catch (err: any) {
      console.error('Error fetching data:', err.message);
      setDiscover([]);
      toast.error(err.message || 'Error fetching data. Please try again later.');
    } finally {
      setLoading(false);
    }
  }, []);
