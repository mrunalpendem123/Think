...
const MessageInput = () => {
  const { loading, sendMessage, replyingTo, setReplyingTo } = useChat();
    ...
    <div className="w-full">
      ...
      <form
        onSubmit={(e) => {
          if (loading) return;
          e.preventDefault();
          console.log('🔵 Form submit - replyingTo:', replyingTo ? 'YES' : 'NO');
          console.log('🔵 Form submit - message:', message);
          sendMessage(message);
          setMessage('');
          setReplyingTo(null);
        }}
        onKeyDown={(e) => {
          if (e.key === 'Enter' && !e.shiftKey && !loading) {
            e.preventDefault();
            console.log('🔵 Enter key - replyingTo:', replyingTo ? 'YES' : 'NO');
            console.log('🔵 Enter key - message:', message);
            sendMessage(message);
            setMessage('');
            setReplyingTo(null);
          }
